...
1/length(a_runs)*sum((a_runs-a_true).^2)
1/length(b_runs)*sum((b_runs-b_true).^2)
...
MES_3_a = 1/length(a_runs)*sum((a_runs-a_true).^2)
MES_3_b = 1/length(b_runs)*sum((b_runs-b_true).^2)
...
for i=1:100
    A = [xpoints.*(1./std_noise) [1; 1; 1].*(1./std_noise)];
    B = ynoisy(:,i).*(1./std_noise);
    res = A\B;
    a_runs(i) = res(1);
    b_runs(i) = res(2);
end

MES_4_a = 1/length(a_runs)*sum((a_runs-a_true).^2)
MES_4_b = 1/length(b_runs)*sum((b_runs-b_true).^2)
