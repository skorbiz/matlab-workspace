clc;clear;close all;
%-----------------------------------------------------------------------------------------------------
% SEQUENTIAL TRANSMISSON OF WAVELET COEFFICIENTS USED FOR ITERATIVE IMPROVED RECONSTRUCTION
%-----------------------------------------------------------------------------------------------------
...
%-------------------------------------------------------------------------------------------------------------
% plot iterative improved reconstructions
figure('Name',['ITERATIVELY IMPROVED RECONSTRUCTION at receiving side of sequentially transmitted image,   analysis with wavelet = ' wavelet , ' to level  = ' num2str(maxlevel)])
subplot(2,maxlevel/2+1,1)
image(wcodemat(I,nbcol));colormap(map);colorbar;axis image,title(['signal']),xlabel(['image = ' num2str(rows*columns) ' bytes, transmitted coeffs = ' num2str(length(C)) ' bytes'])
for j = maxlevel:-1:0
    subplot(2,maxlevel/2+1,maxlevel-j+2)
    A = wrcoef2('a',C,S,wavelet,j);
    if j == maxlevel
        bytes = S(1,1)*S(1,2);
    else
        bytes = bytes + 3*S(maxlevel-j+1,1)*S(maxlevel-j+1,2);
    end
    image(wcodemat(A,nbcol));colormap(map);colorbar;axis image,title(['approximation A' num2str(j)]),xlabel(['received ' num2str(bytes) ' bytes = ' num2str(100*bytes/length(C),4) ' %'])
end
%--------------------------------------------------------------------------------------------------------------
